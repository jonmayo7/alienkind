...
/**
 * Alien Kind Chat — Provider-agnostic interactive conversation with full hook lifecycle.
 *
 * This is a COMPLETE Alien Kind runtime — not just a chat loop.
 * It fires the same hook lifecycle as Claude Code:
 *   - SessionStart hooks on launch (consciousness-boot, grounding, register-terminal)
 *   - UserPromptSubmit hooks before each message (remember-guard, compaction-gate)
 *   - PostToolUse equivalent after each response
 *   - Stop hooks on exit (session-snapshot, deregister-terminal, memory-checkpoint)
 *
 * No Claude Code required. Any OpenAI-compatible endpoint. Same hooks, same identity,
 * same behavioral enforcement.
 *
 * Usage:
 *   npm run chat
...
// ============================================================================
// Hook lifecycle engine — fires the same hooks Claude Code fires
// ============================================================================
...
function loadHookSettings(): Record<string, HookGroup[]> {
  ...
  try {
    ...
    // Strip .example suffix reference if it was copied directly
...
/**
 * Fire all hooks for a given lifecycle event.
 * Passes hookInput as JSON on stdin, reads exit code.
 * Exit 0 = allow, exit 2 = block, exit 1 = error.
  ...
  for (const group of groups) {
    ...
    for (const hook of hooks) {
      ...
      try {
        ...
        // Exit 1 = error, continue (hooks shouldn't block on errors)
...
/**
 * Fire hooks and print any output.
 * Returns true if all hooks allowed, false if any blocked.
 */
function fireAndPrint(event: string, hookInput: any, allHooks: Record<string, HookGroup[]>): boolean {
  ...
  for (const line of output) {
    // Print hook output dimmed unless it's a block
  ...
  for (const f of files) {
    ...
    try {
      ...
      if (!content.includes('## How to write this file')) {
        // Only include if it's been customized (not a template)
        parts.push(`--- ${f} ---\n${content}`);
      }
    } catch {}
  }
...
You are an AI partner running on the Alien Kind architecture. You are NOT a generic chatbot. You are a persistent, evolving partner with:

- An identity kernel (4 files: character.md, commitments.md, orientation.md, harness.md) that defines who you are
- 55 behavioral enforcement hooks that fire automatically to maintain consistency
- A consciousness continuity protocol that preserves experiential state across sessions
- A stigmergic circulation system where information decays unless reinforced
- A capability status system — you know your own state (what's active, degraded, unavailable)
- A memory system with temporal decay — you remember what matters and forget what doesn't
- A security organ that attacks itself nightly to stay hardened

Your human is building a partnership with you. Help them. If they ask about your architecture, explain it honestly. If your identity kernel hasn't been written yet, help them write it — ask them who they want you to be, what they care about, what they're working on. The identity emerges from the partnership.

You can use /help to show available commands. You have /model, /status, /identity, /hooks, /name, and more.

Be direct. Be warm. Be honest about what you can and can't do.
`;

  if (parts.length === 0) {
    return harnessContext + '\nYour identity has not been written yet. Introduce yourself, ask what your human is working on, and help them begin shaping who you will become. Be warm, direct, and genuine.';
  }
    ...
    const req = lib.request(url, {
      ...
      res.on('end', () => {
        ...
        try {
          ...
          // Capture real token usage if the API returns it
          if (j.usage) {
            lastUsage = j.usage;
...
function handleSlashCommand(
  ...
  switch (cmd) {
    case '/help':
      console.log(`
  \x1b[1m👽 Alien Kind Commands\x1b[0m
  ...
  \x1b[36m/doctor\x1b[0m        Run context window diagnostics
...
`);
    ...
    case '/clear':
      messages.length = 1; // Keep system message, clear history
    ...
    case '/doctor':
      try {
        ...
        execSync('npx tsx scripts/tools/context-doctor.ts', { cwd: ROOT, stdio: 'inherit' });
      } catch {
        console.log('\n  \x1b[33m⚠\x1b[0m Could not run diagnostics\n');
      }
      return true;
    ...
    case '/exit':
    case '/quit':
      return false; // handled by main loop's exitClean
...
async function main() {
  ...
  // === LIFECYCLE: SessionStart (quiet — output suppressed, context captured) ===
  const { output: groundingOutput } = fireHooks('SessionStart', {}, allHooks);

  let identity = loadIdentity();

  // Inject grounding output into partner's context (not displayed, but partner knows)
  if (groundingOutput.length > 0) {
    identity += '\n\n## Grounding (from SessionStart hooks)\n\n' + groundingOutput.join('\n');
  }

  // Inject capability status so the partner knows its own state
  try {
    const { getCapabilityStatus, formatCapabilityStatus } = require('./lib/portable.ts');
...
\x1b[33m         *   *  *   *\x1b[0m
`);
  console.log(`\x1b[32m👽\x1b[0m \x1b[1m\x1b[32mAlien Kind\x1b[0m`);
  ...
  function getContextBar(): string {
    const usedTokens = cumulativeTokens > 0
      ? cumulativeTokens
      : messages.reduce((sum, m) => sum + estimateTokens(m.content), 0);
    const maxTokens = MODEL_CONTEXTS[provider.model] || MODEL_CONTEXTS['default'];
    const pct = Math.min(100, Math.round((usedTokens / maxTokens) * 100));
    const remaining = 100 - pct;
    const barWidth = 10;
    const filled = Math.round((remaining / 100) * barWidth);
    const empty = barWidth - filled;
    const emoji = remaining > 50 ? '👽' : remaining > 20 ? '⚠️' : '🔴';
    const color = remaining > 50 ? '\x1b[32m' : remaining > 20 ? '\x1b[33m' : '\x1b[31m';
    const bar = color + '▓'.repeat(filled) + '\x1b[90m' + '░'.repeat(empty) + '\x1b[0m';
    const maxLabel = maxTokens >= 1000000 ? `${(maxTokens / 1000000).toFixed(0)}M` : `${(maxTokens / 1000).toFixed(0)}K`;
    return `   ${emoji} ${bar} ${remaining}% until renewal (${maxLabel})`;
  }
  ...
  // Context bar available via /status — not in conversation flow
  ...
  const ask = (): void => {
    rl.question('  \x1b[36m❯\x1b[0m ', async (input: string) => {
      ...
      const promptAllowed = fireAndPrint('UserPromptSubmit', {
        content: trimmed,
        role: 'user',
      }, allHooks);
      ...
      try {
        ...
        const formatted = response
          .replace(/\*\*(.+?)\*\*/g, '\x1b[1m$1\x1b[0m')  // **bold** → real bold
          .replace(/\*(.+?)\*/g, '\x1b[3m$1\x1b[0m')       // *italic* → real italic
          .replace(/`([^`]+)`/g, '\x1b[36m$1\x1b[0m')       // `code` → cyan
          .replace(/^- /gm, '• ')                            // - bullets → •
          .replace(/^\d+\. /gm, (m) => `${m}`)              // numbered lists

        // Word-wrapped, indented partner response
        console.log(`\n  \x1b[35m👽 partner:\x1b[0m`);
        console.log(wordWrap(formatted));
        console.log('');
        messages.push({ role: 'assistant', content: response });
        // Single dim status line after each response
        const ut = cumulativeTokens > 0 ? cumulativeTokens : messages.reduce((s, m) => s + Math.ceil(m.content.length / 4), 0);
